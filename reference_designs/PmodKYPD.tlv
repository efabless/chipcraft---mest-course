...
\TLV PmodKYPD(/_top, /_name, @_first_stage, $_pmod_in, $_pmod_out, $_ready, $_led_out, $_debug, _where)
   /_name
      ...
      @_first_stage
         ...
         m5_var(SeqWidth, m5_if(m5_MAKERCHIP, 4, 22))    /// 12 Number of bits counting one sample to the next. 22 for 1/4 sec per poll
         m5_var(SampleWidth, m5_if(m5_MAKERCHIP, 2, 20))  /// 7 Number of bits counting sample window.
         m5_var(FullSpeedSeqWidth, m5_if(m5_MAKERCHIP, 4, 12))   /// SeqWidth for full-speed operation.
         m5_var(FullSpeedSampleWidth, m5_if(m5_MAKERCHIP, 2, 7)) /// SampleWidth for full-speed operation.
         ...
         $Seq[m5_calc(m5_SeqWidth - 1 + 2):0] <=
            $reset ? 0 : $Seq + 1;
         $sampling = /_top$debug ? $Seq[m5_calc(         m5_SeqWidth - 1):         m5_SampleWidth] == m5_calc(         m5_SeqWidth -          m5_SampleWidth)'b0 :
                                   $Seq[m5_calc(m5_FullSpeedSeqWidth - 1):m5_FullSpeedSampleWidth] == m5_calc(m5_FullSpeedSeqWidth - m5_FullSpeedSampleWidth)'b0;
         $sample = $sampling &&
                   /_top$debug ? $Seq[m5_calc(         m5_SampleWidth - 1):0] == ~          m5_SampleWidth'b0 :
